...
│                                                   # -v .:/usr/local/structurizr structurizr/structurizr`
├── .github/
...
- **Краткое обоснование (для тела ADR при создании файла):**
  ...
  - **Confirmation:** workspace.dsl лежит в корне; `docker run -it --rm -p 8080:8080 -v .:/usr/local/structurizr structurizr/structurizr` поднимается без ошибок; views `systemContext`, `container`, `component` рендерятся; `properties { "adr-link" ... }` присутствует у ключевых элементов; CI-линт DSL (опционально) — отдельная задача roadmap.
...
**DoD:** PR создан; `docker run structurizr/structurizr` поднимает пустую модель без ошибок.
