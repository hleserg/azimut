workspace "Азимут" "ИИ-ассистент по коду 1С на базе форка bsl-atlas (ADR 0034)" {
    ...
    model {
        ...
        #   Технология (sync если не помечено иначе):
        #     "Python API"      — кросс-контейнерный in-process вызов (Python import + function call)
        #     "in-process"      — компонент → компонент в одном контейнере
        #     "ChromaDB API"    — sync, embedded ChromaDB
        #     "SQLite API"      — sync, sqlite3 driver
        #     "MCP / JSON-RPC"  — sync request-response (stdio/http)
        #     "HTTPS / X"       — sync request-response
        #     "HTTPS / X (fire-and-forget)"  — async, без ожидания ответа (Sentry SDK)
        #     "Python API (async background)" — через FastAPI BackgroundTask
        ...
        azimuth.mcpServer -> azimuth.parsers "MetadataXMLParser для SQLite rebuild" "Python API"
        azimuth.mcpServer -> azimuth.indexer "init, MCP reindex (sync), HTTP /reindex (async background)" "Python API"
        azimuth.mcpServer -> azimuth.searchEngine "семантический поиск (codesearch/helpsearch) + code_grep fallback" "Python API"
        azimuth.mcpServer -> azimuth.storage "структурные запросы (символы, граф, метаданные)" "Python API"
        azimuth.mcpServer -> azimuth.antiHall "вход в anti-hallucination pipeline (планируется)" "Python API"
        azimuth.mcpServer -> azimuth.llmAdapter "разговорная LLM для финального ответа и дип-ресёрча (планируется)" "Python API"

        # ── Индексатор → нижележащие ───────────────────────────────────────────
        azimuth.indexer -> azimuth.parsers "парсит BSL, текстовые дампы метаданных, HTML-справку" "Python API"
        azimuth.indexer -> azimuth.vectorStore "collection.add (batch), collection.delete" "ChromaDB API"
        azimuth.indexer -> azimuth.fileTrackerDb "SHA-256 хеши, статусы файлов и функций" "SQLite API"

        # ── Поисковый движок → нижележащие ─────────────────────────────────────
        azimuth.searchEngine -> azimuth.vectorStore "collection.query (семантика + fulltext через where_document)" "ChromaDB API"
        azimuth.searchEngine -> azimuth.storage "code_grep по FTS5 (только CodeGrep)" "Python API"
        azimuth.searchEngine -> rerankerService "POST /rerank (опц., timeout 5s)" "HTTPS / JSON"

        # ── Хранилище → SQLite + парсеры ───────────────────────────────────────
        azimuth.storage -> azimuth.sqlite "DDL, INSERT/UPDATE/DELETE, SELECT (symbols, calls, objects, FTS5)" "SQLite API"
        azimuth.storage -> azimuth.parsers "CodeParser.parse_file_functions внутри SQLiteStore.rebuild()" "Python API"

        # ── Anti-Hallucination Pipeline → внешние ──────────────────────────────
        azimuth.antiHall -> azimuth.llmAdapter "дип-ресёрч в фолбэке (ADR 0007)" "Python API"
        azimuth.antiHall -> sentry "лог уровней groundedness 2/3 (ADR 0008)" "HTTPS / Sentry SDK (fire-and-forget)"

        # ── Adapter-слой LLM → внешние LLM ─────────────────────────────────────
        azimuth.llmAdapter -> deepSeekLLM "разговорная генерация (дефолт)" "HTTPS / OpenAI-compatible API"
        azimuth.llmAdapter -> claudeLLM "премиум-генерация (Сергей-премиум)" "HTTPS / Anthropic API"

        # ── MCP-сервер → внешние сервисы ───────────────────────────────────────
        azimuth.mcpServer -> bslPlatformMcp "запрашивает справочник синтаксиса платформы" "MCP / JSON-RPC"
        azimuth.mcpServer -> sentry "трассировки и unhandled exceptions" "HTTPS / Sentry SDK (fire-and-forget)"
        azimuth.mcpServer -> its "поиск справочных материалов (3-й уровень иерархии, ADR 0006)" "HTTPS"
        ...
        azimuth.mcpServer.fastmcpApp -> azimuth.mcpServer.appConfig "читает env-настройки и dataclass-конфиг" "in-process"
        azimuth.mcpServer.fastmcpApp -> azimuth.storage.sqliteStore "search_function, get_module_functions, get_function_context, get_object_details, metadatasearch, code_grep (FTS5)" "Python API"
        azimuth.mcpServer.fastmcpApp -> azimuth.indexer.vectorIndexer "init_services, reindex MCP tool (sync); HTTP /reindex" "Python API (async background)"
        azimuth.mcpServer.fastmcpApp -> azimuth.indexer.embeddingProviders "create_embedding_provider для search/reindex провайдеров" "Python API"
        azimuth.mcpServer.fastmcpApp -> azimuth.searchEngine.hybridSearch "codesearch, helpsearch, search_code_filtered" "Python API"
        azimuth.mcpServer.fastmcpApp -> azimuth.searchEngine.codeGrep "code_grep (fallback на сканирование файлов)" "Python API"
        azimuth.mcpServer.fastmcpApp -> azimuth.parsers.metadataXMLParser "parse_directory(source) при SQLite rebuild" "Python API"
        azimuth.mcpServer.fastmcpApp -> azimuth.antiHall.querySufficiencyGate "точка входа в anti-hallucination pipeline (планируется)" "Python API"

        # ── Парсеры компоненты ─────────────────────────────────────────────────
        azimuth.parsers.bslCodeParser -> azimuth.parsers.treeSitterParser "AST через tree-sitter с regex fallback" "in-process"
        azimuth.parsers.bslCodeParser -> azimuth.storage.dataModels "возвращает BSLFunction" "Python API"
        azimuth.parsers.metadataXMLParser -> azimuth.storage.dataModels "возвращает MetadataObject, Attribute, TabPart" "Python API"

        # ── Индексатор компоненты ──────────────────────────────────────────────
        azimuth.indexer.vectorIndexer -> azimuth.indexer.embeddingProviders "embed_documents (batch чанков, с retry)" "in-process"
        azimuth.indexer.vectorIndexer -> azimuth.indexer.fileTracker "get_file_status, mark_indexed, mark_function_indexed" "in-process"
        azimuth.indexer.vectorIndexer -> azimuth.parsers.bslCodeParser "parse_file_functions (function-level чанкинг)" "Python API"
        azimuth.indexer.vectorIndexer -> azimuth.parsers.metadataTextParser "parse_file (текстовые дампы метаданных, skip при sqlite_enabled=true)" "Python API"
        azimuth.indexer.vectorIndexer -> azimuth.parsers.helpParser "parse_file (HTML-справка → markdown)" "Python API"
        azimuth.indexer.vectorIndexer -> azimuth.vectorStore "collection.add (batch до 5000, retry на failures)" "ChromaDB API"
        azimuth.indexer.fileTracker -> azimuth.fileTrackerDb "файлы (path, hash, status) + function-hashes" "SQLite API"

        # ── Поисковый движок компоненты ────────────────────────────────────────
        azimuth.searchEngine.hybridSearch -> azimuth.vectorStore "collection.query (vector / where_document)" "ChromaDB API"
        azimuth.searchEngine.hybridSearch -> azimuth.indexer.embeddingProviders "embed_query (если задан отдельный search_provider)" "Python API"
        azimuth.searchEngine.hybridSearch -> rerankerService "POST /rerank (опц., timeout 5s, fail-open)" "HTTPS / JSON"
        azimuth.searchEngine.codeGrep -> azimuth.storage.sqliteStore "sqlite_store.code_grep по FTS5 (если populated)" "Python API"

        # ── Хранилище компоненты ───────────────────────────────────────────────
        azimuth.storage.sqliteStore -> azimuth.sqlite "DDL (schema/FTS5), INSERT/SELECT (symbols, calls, objects, code_fts)" "SQLite API"
        azimuth.storage.sqliteStore -> azimuth.parsers.bslCodeParser "parse_file_functions внутри SQLiteStore.rebuild()/update()" "Python API"
        azimuth.storage.sqliteStore -> azimuth.storage.dataModels "возвращает FunctionInfo, FunctionContext, MetadataInfo, ObjectDetails" "in-process"

        # ── Anti-Hallucination Pipeline компоненты ─────────────────────────────
        azimuth.antiHall.querySufficiencyGate -> azimuth.antiHall.serverControlledRetrieval "осмысленный запрос → ретривинг" "in-process"
        azimuth.antiHall.serverControlledRetrieval -> azimuth.searchEngine "ретривинг с планкой релевантности и потолком окна" "Python API"
        azimuth.antiHall.serverControlledRetrieval -> azimuth.antiHall.sourceHierarchy "результаты ретривинга → ранжирование по иерархии" "in-process"
        azimuth.antiHall.sourceHierarchy -> azimuth.antiHall.contradictionMetric "ранжированные источники → проверка противоречивости" "in-process"
        azimuth.antiHall.contradictionMetric -> azimuth.antiHall.llmJudge "спорные источники → арбитр" "in-process"
        azimuth.antiHall.llmJudge -> claudeLLM "арбитраж faithfulness/groundedness со спан-привязкой" "HTTPS / Anthropic API"
        azimuth.antiHall.llmJudge -> azimuth.antiHall.groundednessDetector "сигнал недогрунтованности (relevance high / groundedness low)" "in-process"
        azimuth.antiHall.groundednessDetector -> azimuth.antiHall.reRetrievalController "уровень 1: триггер повторного прохода" "in-process"
        azimuth.antiHall.groundednessDetector -> sentry "уровни 2/3: лог недогрунтованности" "HTTPS / Sentry SDK (fire-and-forget)"
        azimuth.antiHall.reRetrievalController -> azimuth.antiHall.serverControlledRetrieval "повтор в рамках бюджета окна" "in-process"
        azimuth.antiHall.reRetrievalController -> azimuth.antiHall.fallbackMode "N повторов исчерпано → смена режима (ADR 0007)" "in-process"
        azimuth.antiHall.fallbackMode -> azimuth.llmAdapter.conversationalAdapter "дип-ресёрч с тем же контрактом (ADR 0007)" "Python API"

        # ── Adapter-слой LLM компонент ─────────────────────────────────────────
        azimuth.llmAdapter.conversationalAdapter -> deepSeekLLM "разговорная генерация (дефолт)" "HTTPS / OpenAI-compatible API"
        azimuth.llmAdapter.conversationalAdapter -> claudeLLM "премиум-генерация и арбитраж для LLM Judge" "HTTPS / Anthropic API"
    }
