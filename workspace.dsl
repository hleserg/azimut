workspace "Азимут" "ИИ-ассистент по коду 1С на базе форка bsl-atlas (ADR 0034)" {
    ...
    model {
        ...
        azimuth.mcpOrchestrator.reRetrieval -> azimuth.mcpOrchestrator.fallbackMode "переключает в дип-ресёрч после N исчерпанных повторов (Р7, ADR 0007/0009)" "in-process"
        azimuth.mcpOrchestrator.fallbackMode -> azimuth.llmAdapter "инициирует дип-ресёрч" "Python API"
        azimuth.mcpOrchestrator.routeDispatcher -> azimuth.llmAdapter "запрашивает синтез ответа через разговорную модель (ADR 0021)" "Python API"
        azimuth.llmAdapter -> claudeLLM "генерирует текст в премиум-режиме (Сергей-премиум)" "HTTPS / Anthropic API"
    }
    ...
    views {
        ...
        styles {
            ...
            # Structurizr вешает теги "Decision:<Status>" на ADR-узлы на основе
            # поля `status:` из frontmatter, **as-is**. У нас в frontmatter
            # lowercase ("accepted", "proposed", "superseded by 0019"), поэтому
            # стилизуем именно в lowercase + базовые статусы Title Case для
            # совместимости с дефолтным примером Structurizr.
